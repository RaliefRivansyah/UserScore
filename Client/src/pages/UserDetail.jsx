        ...
        <div className="min-h-screen bg-slate-900 p-8">
            <div className="max-w-4xl mx-auto">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-3xl font-bold text-slate-200">User Detail</h1>
                ...
                <div className="bg-slate-800 rounded-lg shadow-sm p-6 mb-6 border border-slate-700">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p className="font-medium text-slate-200">{user.birth_place || '-'}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p className="font-medium text-slate-200">{user.gender || '-'}</p>
                        </div>
                        <div>
                            <p className="text-sm text-slate-400">Postal Code</p>
                            <p className="font-medium text-slate-200">{user.postal_code || '-'}</p>
                        </div>
                ...
                <div className="bg-slate-800 rounded-lg shadow-sm p-6 mb-6 text-center border border-slate-700">
                    <h2 className="text-xl font-bold mb-6 text-slate-200">Scoring Result</h2>
                    <div className="flex justify-center items-center gap-12">
                        <div>
                            <p className="text-sm text-slate-400 mb-1">Total Score</p>
                            <p className="text-4xl font-bold text-blue-500">{user.total_score}</p>
                        </div>
                ...
                <div className="bg-slate-800 rounded-lg shadow-sm p-6 border border-slate-700">
                    <h2 className="text-xl font-bold mb-6 text-slate-200">Selected Scoring Items</h2>
                    {user.user_answers && user.user_answers.length > 0 ? (
                        <div className="space-y-8">
                            {Object.entries(
                                ...
                                <div key={groupName}>
                                    <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3">{groupName}</h3>
                                    <div className="bg-slate-900 rounded-lg p-4 space-y-3 border border-slate-700">
                                        {answers.map((answer, index) => (
                                            <div
                                                key={index}
                                                className="flex justify-between items-start border-b border-slate-700 last:border-0 pb-3 last:pb-0">
                                                <div>
                                                    <p className="text-sm text-slate-400 mb-1">{answer.group_item?.name}</p>
                                                    <p className="font-medium text-slate-200">{answer.item?.label}</p>
                                                </div>
                                                <div className="text-right">
                                                    <span className="text-lg font-bold text-blue-500">{answer.item?.value}</span>
                        ...
                        <p className="text-slate-400 text-center py-8">No answers recorded</p>
                    )}
                </div>
            </div>
        </div>
